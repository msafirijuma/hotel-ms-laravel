...
class HousekeepingController extends Controller
{
    ...
    {
        $request->validate([
            'issue_notes' => 'required|string|max:500'
        ]);

        DB::transaction(function () use ($request, $task) {
            // Status cleaning completed with maintenance issue
            $task->update([
                'status' => 'completed',
                'completed_at' => now(),
                'description' => $task->notes . ' | Maintenance: ' . trim($request->issue_notes)
            ]);

            // room status to maintenance (not available)
            $task->room->update([
                'status' => 'maintenance',
                'description' => 'Cleaning with report: ' . trim($request->issue_notes)
            ]);

            LogActivity::log('Maintenance', 'Room No. ' . $task->room->room_number . ' has been reported for maintanance issue: ' . $request->issue_notes);
        });

        return redirect()->route('dashboard')->with('success', 'Maintenance issue reported to Admin. Room is under maintenance!');
    }

    /**
     * All room with status maintenace (Admin and Receptionist only)
     */
    ...
    /**
     * Fixing issue (Admin/Technician Only - Mark Fixed Button)
     */
